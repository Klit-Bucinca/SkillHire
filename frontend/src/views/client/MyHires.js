...
function HireRow({ hire }) {
  ...
  const photoUrl = p?.profilePhoto
    ? (p.profilePhoto.startsWith("http") ? p.profilePhoto : `${backendUrl}${p.profilePhoto}`)
    : "/img/placeholder.jpg";
  ...
  return (
    <div className="bg-white border border-blueGray-100 rounded-lg shadow p-4 flex items-center justify-between">
      <div className="flex items-center gap-4 min-w-0">
        <img src={photoUrl} alt="avatar" className="w-40 h-40 rounded-md object-cover" />
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            <h3 className="font-semibold text-blueGray-700 truncate">{name}</h3>
            <span className={`px-2 py-0.5 rounded-full text-[10px] font-semibold ${badgeClass(status)}`}>{status}</span>
          </div>
          <p className="text-xs text-blueGray-500">
            {city} • {when.toLocaleDateString()} {" "}
            <span className="text-blueGray-400">{when.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</span>
          </p>
          ...
          {hire.notes && <p className="text-xs text-blueGray-600 mt-1 truncate">Notes: {hire.notes}</p>}
        </div>
      </div>
      {p?.photos?.length > 0 && (
        <div className="hidden sm:flex gap-1 ml-4">
          {p.photos.slice(0, 2).map((ph) => (
            <img
              key={ph.id}
              src={ph.imageUrl.startsWith("http") ? ph.imageUrl : `${backendUrl}${ph.imageUrl}`}
              alt="work"
